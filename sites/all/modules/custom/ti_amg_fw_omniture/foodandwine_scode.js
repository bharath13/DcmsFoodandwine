/* SiteCatalyst code version: H.26. Copyright 1996-2013 Adobe, Inc. All Rights Reserved. More info available at http://www.omniture.com (2016-02-17)*/
...
if(channel!='slideshowsHD' && channel!='video'){
...
/* Optimizely Custom Omniture Integration: populates active experiments to evar_slot if evar_slot not set in UI */
var evar_slot = 61;
...
/* controlling the video player in the hero carousel */
  if (s_time.prop15 && (s_time.prop15 == "video_landing_page")) {
    videoPlayer.play(); 
  }
  if (s_time.prop15 && ((s_time.prop15 == "Home Page") || (s_time.prop15 == "channel_landing_page"))) {
    if (event.target.experience.id == "myExperience") { console.log("video is not playing on the page loading"); }
    else {
      videoPlayer.play(); 
    }    
  }
...
var playingPlayer = brightcove.api.getExperience(evt.target.experience.id);
  for (var i = 0; i < players.length; i++) {
    if (players[i] == id) {
	console.log("iteration of players");
